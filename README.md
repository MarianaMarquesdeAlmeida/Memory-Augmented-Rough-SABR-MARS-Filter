...
denote the panel of option quotes observed at timestamp $t$, where $K_{t,j}$ is the strike, $T_{t,j}$ is the remaining time to expiry, and $m_t$ is the number of selected option contracts.
...
* $\rho_t$ is the correlation parameter;
...
The MARS model augments the particle state with a finite-dimensional representation of the rough-volatility memory:
...
notebooks/      Model comparison and empirical analysis
```
...
The examples below use $100$ timestamps, $100$ particles, $128$ Monte Carlo pricing paths, and a maximum of $30$ option quotes per timestamp. These settings are appropriate for an initial test run. Larger experiments require substantially more computation.
...
\begin{cases}
P^{\mathrm{bid}}*{t,j}-\widehat{P}*{t,j},
&
\widehat{P}*{t,j}<P^{\mathrm{bid}}*{t,j},
[4pt]
0,
&
P^{\mathrm{bid}}*{t,j}
...
P^{\mathrm{ask}}*{t,j},
[4pt]
\widehat{P}*{t,j}-P^{\mathrm{ask}}*{t,j},
&
\widehat{P}*{t,j}>P^{\mathrm{ask}}_{t,j}.
\end{cases}
$$
...
The example commands are therefore intended as test configurations. Full empirical experiments should be run using larger particle and Monte Carlo samples only after verifying that the data paths and model outputs are correct.
